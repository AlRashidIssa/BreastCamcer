    ...
    if request.method == 'POST':
        if 'csv-file' in request.FILES:
            ...
            if csv_file.name.endswith('.csv'):
                ...
                return redirect('results')  # Adjust this based on your URL setup
            ...
            return redirect('results')  # This redirects to the 'results' URL pattern
...
def result(request):
    ...
    try:
        ...
        explanation_html = markdown.markdown(explanation) # type: ignore

        return render(request, 'result.html', {'predictions': predictions,
                                               'massage_promet': explanation_html})
    except (FileNotFoundError, ValueError) as e:
        return render(request, 'errors.html', {'error': str(e)})
