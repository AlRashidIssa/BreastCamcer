...
sys.path.append("/home/alrashidissa/Desktop/")
...
class Download(IDownload):
    ...
    def call(self, url: str, output_path: str, name_dataset: str = "brestcancerset") -> None:
        ...
        try:
            ...
            info(f"Starting download from {download_url}")
            gdown.download(download_url, str(file_path), quiet=False)

            # Check the size of the downloaded file
            file_size = Size().call(file_path)
            info(f"Download completed. File size: {file_size}")

        except Exception as e:
            critical(f"An error occurred during the download: {e}")
            raise
